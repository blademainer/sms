...
class IPModemDriver extends AModemDriver {
	...
	private TerminalTypeOptionHandler ttopt = new TerminalTypeOptionHandler(
			"VT100", false, false, true, false);

	private SimpleOptionHandler binaryopt = new SimpleOptionHandler(0, true,
			false, true, false);

	private EchoOptionHandler echoopt = new EchoOptionHandler(true, false,
			true, false);

	private SuppressGAOptionHandler gaopt = new SuppressGAOptionHandler(true,
			true, true, true);
	...
	protected void connectPort() throws GatewayException, IOException,
		...
		try {
			Logger.getInstance().logInfo(
					"Opening: " + this.ipAddress + " @" + this.ipPort, null,
			...
			if (getGateway().getIpProtocol() == IPProtocols.BINARY)
				this.tc.addOptionHandler(this.binaryopt); // Make telnet session
															// binary, so ^Z in
															// ATHander.Sendmessage
															// is send raw!
			if (getGateway().getIpEncryption()) {
				try {
					this.tc.setSocketFactory(SSLContext.getInstance("Default")
							.getSocketFactory());
				} catch (NoSuchAlgorithmException e) {
					Logger.getInstance().logError(
							"Unable to find algorithm needed for using SSL", e,
							getGateway().getGatewayId());
					// TODO: although not supposed to happen, something should
					// be done if it does
				}
			}
	...
	protected void disconnectPort() throws IOException, InterruptedException {
		Logger.getInstance().logInfo(
				"Closing: " + this.ipAddress + " @" + this.ipPort, null,
				getGateway().getGatewayId());
	...
	private class Peeker extends Thread {
		...
		@Override
		public void run() {
			Logger.getInstance().logDebug("Peeker started.", null,
					getGateway().getGatewayId());
			while (true) {
				try {
					if (getTc() != null) {
			...
			Logger.getInstance().logDebug("Peeker stopped.", null,
					getGateway().getGatewayId());
		}
	}
}
