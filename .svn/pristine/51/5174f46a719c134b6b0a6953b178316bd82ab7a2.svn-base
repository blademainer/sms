...
class HTTPGateway extends AGateway {
		...
		for (int i = 0, n = requestList.size(); i < n; i++) {
			...
			if (requestList.get(i).unicode) {
				...
				for (int j = 0; j < uniBytes.length; j++)
					tmp.append(Integer.toHexString(uniBytes[j]).length() == 1 ? "0"
	...
	String calculateMD5(String in) {
		try {
			...
			Logger.getInstance().logError("Unsupported encoding.", ex,
					getGatewayId());
			return "";
		} catch (NoSuchAlgorithmException ex) {
			Logger.getInstance().logError("No such algorithm.", ex,
					getGatewayId());
			return "";
		}
	}
