...
public class Http extends Interface<Integer> {
	public Http(String myInterfaceId, Properties myProps, SMSServer myServer,
	...
	public void messagesReceived(Collection<InboundMessage> msgList)
		...
		Logger.getInstance().logInfo("SMSServer: Interface HTTP: " + template,
				null, null);
		for (InboundMessage im : msgList) {
			if (template != null
					&& ((im.getType() == MessageTypes.INBOUND) || (im.getType() == MessageTypes.STATUSREPORT))) {
				String getURL = updateInboundTemplateString(template, im);
				Logger.getInstance().logInfo(
						"SMSServer: Interface HTTP: " + getURL, null, null);
	...
	List<String> HttpDo(URL url) throws IOException {
		if ("POST".equalsIgnoreCase(getProperty("method")))
			return HttpPost(url);
		return HttpGet(url);
	}
	...
	List<String> HttpPost(URL url) throws IOException {
		...
		URL cleanUrl = new URL(url.getProtocol(), url.getHost(), url.getPort(),
				url.getPath());
	...
	private String updateInboundTemplateString(String template,
			InboundMessage msg) throws UnsupportedEncodingException {
		ExtStringBuilder sb = new ExtStringBuilder(template);
		sb.replaceAll(
				"%gatewayId%",
				URLEncoder.encode(msg.getGatewayId(),
						getProperty("encoding", "ISO-8859-1")));
		sb.replaceAll(
				"%encoding%",
				(msg.getEncoding() == MessageEncodings.ENC7BIT ? "7-bit" : (msg
						.getEncoding() == MessageEncodings.ENC8BIT ? "8-bit"
						: "UCS2 (Unicode)")));
		if (msg.getDate() != null)
			sb.replaceAll(
					"%date%",
					URLEncoder.encode(msg.getDate().toString(),
							getProperty("encoding", "ISO-8859-1")));
		sb.replaceAll(
				"%text%",
				URLEncoder.encode(msg.getText(),
						getProperty("encoding", "ISO-8859-1")));
	...
	private String updateOutboundTemplateString(String template,
			OutboundMessage msg) throws UnsupportedEncodingException {
		ExtStringBuilder sb = new ExtStringBuilder(template);
		sb.replaceAll(
				"%gatewayId%",
				URLEncoder.encode(msg.getGatewayId(),
		...
		sb.replaceAll(
				"%refNo%",
				URLEncoder.encode(msg.getRefNo(),
						getProperty("encoding", "ISO-8859-1")));
		sb.replaceAll("%recipient%", msg.getRecipient());
		sb.replaceAll("%from%", msg.getFrom());
		sb.replaceAll("%failureCause%", URLEncoder.encode(msg.getFailureCause()
				.toString(), getProperty("encoding", "ISO-8859-1")));
