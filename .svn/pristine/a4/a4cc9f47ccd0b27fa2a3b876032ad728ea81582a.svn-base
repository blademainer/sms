...
public class ATHandler_Huawei_E160 extends ATHandler_Huawei {
	 ...
	 */
	@Override
	protected String formatUSSDCommand(String presentation, String ussdCommand,
			String dcs) {
		byte[] textSeptets = PduUtils.stringToUnencodedSeptets(ussdCommand);
		byte[] alphaNumBytes = PduUtils
				.unencodedSeptetsToEncodedSeptets(textSeptets);
	...
	public String formatUSSDResponse(String ussdResponse) {
		...
		byte[] responseUnencodedSeptets = PduUtils
				.encodedSeptetsToUnencodedSeptets(responseEncodedSeptets);
