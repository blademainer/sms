...
public abstract class AbstractQueueManager {
	...
	public abstract Collection<OutboundMessage> getPendingMessages(
			String gatewayId);
	...
	public void start() {
		if (delayQueueManager == null || delayQueueManager.isCanceled()) {
			delayQueueManager = new DelayQueueManager("DelayQueueManager",
					queueDelay);
		}
	}
	...
	class DelayQueueManager extends AServiceThread {
		...
		public void process() throws Exception {
			Logger.getInstance().logDebug("DelayQueueManager running...", null,
					null);
			OutboundMessage message = pollDelayedMessage();
			if (message != null)
				queueMessage(message);
			// queueMessage(getDelayQueue().take().getMsg());
			Logger.getInstance().logDebug("DelayQueueManager end...", null,
					null);
		}
	}
}
