...
public abstract class AGateway {
	...
	public void setInbound(boolean value) {
		if ((this.attributes & GatewayAttributes.RECEIVE) != 0)
			this.inbound = value;
	}
	...
	public void setOutbound(boolean value) {
		if ((this.attributes & GatewayAttributes.SEND) != 0)
			this.outbound = value;
	}
	...
	public void setStatus(GatewayStatuses myStatus) {
		Service.getInstance()
				...
				.getNotifyQueue()
				.add(new GatewayStatusNotification(getMyself(), getStatus(),
						myStatus));
	...
	public void startGateway() throws TimeoutException, GatewayException,
		...
		this.queueManager = new QueueManager("QueueManager [" + this.gatewayId
	...
	boolean conformsTo(int attrib, boolean required) {
		if (required && !isCapableOf(attrib))
			return false;
		return true;
	}
		...
		public void process() throws Exception {
			...
			try {
				if (getStatus() == GatewayStatuses.STARTED) {
					msg = Service.getInstance().getQueueManager()
							.pollPendingMessage(getGatewayId());
					if (msg != null) {
						if (Service.getInstance().getQueueSendingNotification() != null)
							Service.getInstance().getQueueSendingNotification()
									.process(getMyself(), msg);
						try {
							if (!sendMessage(msg)) {
								if (msg.getRetryCount() < Service.getInstance()
										.getSettings().QUEUE_RETRIES) {
									Logger.getInstance().logInfo(
											"Reinserting message to queue.",
											null, getGatewayId());
									...
									Logger.getInstance()
											.logWarn(
													"Maximum number of queue retries exceeded, message lost.",
													null, getGatewayId());
									msg.setFailureCause(FailureCauses.UNKNOWN);
									Service.getInstance()
											.getNotifyQueueManager()
											.getNotifyQueue()
											.add(new OutboundMessageNotification(
													getMyself(), msg));
								}
							} else {
								Service.getInstance()
				...
				if ((msg != null)
						&& (msg.getMessageStatus() != MessageStatuses.SENT))
					Service.getInstance().getQueueManager().queueMessage(msg);
				Logger.getInstance().logInfo("QueueManager interrupted.", e,
						getGatewayId());
			} catch (Exception e) {
				Logger.getInstance().logWarn(
						"Queue exception, marking gateway for reset.", e,
						getGatewayId());
				setStatus(GatewayStatuses.RESTART);
				Service.getInstance().getNotifyQueueManager().getNotifyQueue()
						.add(new OutboundMessageNotification(getMyself(), msg));
			}
		}
	}
