...
public class Service {
	...
	private void listSystemInformation() {
		Logger.getInstance().logInfo(Library.getLibraryDescription(), null,
				null);
		Logger.getInstance().logInfo("Version: " + Library.getLibraryVersion(),
				null, null);
		Logger.getInstance().logInfo(
				"JRE Version: " + System.getProperty("java.version"), null,
				null);
		Logger.getInstance().logInfo(
				"JRE Impl Version: " + System.getProperty("java.vm.version"),
				null, null);
		Logger.getInstance().logInfo(
	...
	public void addGateway(AGateway gateway) throws GatewayException {
		if (getServiceStatus() != ServiceStatus.STOPPED)
			throw new GatewayException(
					"Cannot add gateways while Service is running!");
	...
	public boolean removeGateway(AGateway gateway) throws GatewayException {
		if (getServiceStatus() != ServiceStatus.STOPPED)
			throw new GatewayException(
					"Cannot remove gateways while Service is running!");
	 ...
	 * @see #stopService()
	 */
	public synchronized void startService() throws SMSLibException,
	...
	public synchronized void startService(boolean startAllGateways)
			throws SMSLibException, TimeoutException, GatewayException,
		...
		try {
			...
			setWatchDog(new WatchDog("WatchDog",
					(getSettings().WATCHDOG_INTERVAL * 1000)));
	...
	private void startService_Internal(boolean startAll)
		...
		class Starter extends Thread {
			...
			public Starter(AGateway gateway, List<ExceptionPair> eList,
					List<AGateway> gList) {
		...
		if (getSettings().CONCURRENT_GATEWAY_START) {
			...
			while ((gList.size() != getGateways().size())
					&& (eList.size() == 0))
				Thread.sleep(1000);
	 ...
	 * .NET bridge method.
	 */
	public InboundMessage[] readMessages(MessageClasses msgClass)
	 ...
	 */
	public int readMessages(Collection<InboundMessage> msgList,
			MessageClasses msgClass, String gatewayId) throws TimeoutException,
		...
		if ((gateway != null) && (gateway.isInbound())) {
			try {
				...
				Logger.getInstance().logWarn(
						"readMessages(): Gateway " + gateway.getGatewayId()
								+ " does not respond, marking for restart.",
						null, null);
				gateway.setStatus(GatewayStatuses.RESTART);
			} catch (IOException e) {
	 ...
	 */
	public InboundMessage[] readMessages(MessageClasses msgClass,
			String gatewayId) throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		ArrayList<InboundMessage> messageList = new ArrayList<InboundMessage>();
	 ...
	 */
	public int readMessages(Collection<InboundMessage> msgList,
			MessageClasses msgClass, AGateway gateway) throws TimeoutException,
			GatewayException, IOException, InterruptedException {
		if (getServiceStatus() != ServiceStatus.STARTED)
			return 0;
		try {
			...
			Logger.getInstance().logWarn(
					"readMessages(): Gateway " + gateway.getGatewayId()
							+ " does not respond, marking for restart.", null,
					null);
			gateway.setStatus(GatewayStatuses.RESTART);
		} catch (IOException e) {
			Logger.getInstance().logWarn(
					"readMessages(): Gateway " + gateway.getGatewayId()
							+ " throws IO errors, marking for restart.", null,
					null);
			gateway.setStatus(GatewayStatuses.RESTART);
		}
		return msgList.size();
	 ...
	 */
	public InboundMessage[] readMessages(MessageClasses msgClass,
			AGateway gateway) throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		ArrayList<InboundMessage> messageList = new ArrayList<InboundMessage>();
	 ...
	 *             The call was interrupted.
	 */
	public InboundMessage readMessage(String gatewayId, String memLoc,
			int memIndex) throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		if (getServiceStatus() != ServiceStatus.STARTED)
			return null;
	...
	public boolean sendMessage(OutboundMessage msg) throws TimeoutException,
		...
		if (gateway != null) {
			try {
				...
				Logger.getInstance().logWarn(
						"sendMessage(): Gateway " + gateway.getGatewayId()
								+ " does not respond, marking for restart.",
						null, null);
				gateway.setStatus(GatewayStatuses.RESTART);
	...
	public boolean deleteMessage(InboundMessage msg) throws TimeoutException,
		...
		if (gateway != null) {
			try {
				...
				Logger.getInstance().logWarn(
						"deleteMessage(): Gateway " + gateway.getGatewayId()
								+ " does not respond, marking for restart.",
						null, null);
				gateway.setStatus(GatewayStatuses.RESTART);
				return false;
	...
	public boolean sendUSSDRequest(USSDRequest request, String gatewayId)
		...
		if (request == null) {
			throw new IllegalArgumentException(
					"Cannot use a null request object");
		}
		if (gateway == null) {
	...
	public void setInboundMessageNotification(
			IInboundMessageNotification inboundNotification) {
	...
	public void setOutboundMessageNotification(
			IOutboundMessageNotification outboundNotification) {
	 ...
	 */
	public void setGatewayStatusNotification(
			IGatewayStatusNotification gatewayStatusNotification) {
	...
	public void setQueueSendingNotification(
			IQueueSendingNotification queueSendingNotification) {
	 ...
	 */
	public void setOrphanedMessageNotification(
			IOrphanedMessageNotification orphanedMessageNotification) {
			...
			for (AGateway gateway : getGateways()) {
				if (gateway.getStatus() == GatewayStatuses.RESTART) {
					...
					try {
						...
						gateway.setStatus(GatewayStatuses.RESTART);
						Logger.getInstance().logWarn(
								"Error while shutting down Gateway: "
										+ gateway.getGatewayId(), e, null);
					}
					try {
						gateway.startGateway();
					} catch (Exception e) {
