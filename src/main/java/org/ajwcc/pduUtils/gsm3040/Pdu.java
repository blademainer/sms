...
public abstract class Pdu {
	...
	protected void setFirstOctetField(int fieldName, int fieldValue,
			int[] allowedValues) {
	...
	public void setTpMti(int value) {
		setFirstOctetField(PduUtils.TP_MTI_MASK, value, new int[] {
				PduUtils.TP_MTI_SMS_DELIVER, PduUtils.TP_MTI_SMS_STATUS_REPORT,
				PduUtils.TP_MTI_SMS_SUBMIT });
	}
	...
	public void setTpUdhi(int value) {
		setFirstOctetField(PduUtils.TP_UDHI_MASK, value, new int[] {
				PduUtils.TP_UDHI_NO_UDH, PduUtils.TP_UDHI_WITH_UDH });
	}
	...
	public void setDataCodingScheme(int encoding) {
		...
		case PduUtils.DCS_ENCODING_UCS2:
			break;
		default:
			throw new RuntimeException("Invalid encoding value: "
					+ PduUtils.byteToPdu(encoding));
		}
		...
		if (!hasTpUdhi()) {
			...
			case UDH_CHECK_MODE_EXCEPTION_IF_NONE:
				throw new IllegalStateException(
						"PDU does not have a UDHI in the first octet");
	...
	public void setDecodedText(String decodedText) {
		...
		boolean flash = false;
		if (PduUtils.extractDcsFlash(dataCodingScheme) == PduUtils.DCS_MESSAGE_CLASS_FLASH)
			flash = true;
		// clears the coding group to be text again in case it was originally
		// binary
	...
	public byte[] getUserDataAsBytes() {
		...
		System.arraycopy(udData, getTotalUDHLength(), retVal, 0,
				remainingLength);
		return retVal;
	}
	...
	private String decodeNonUDHDataAsString() {
		...
		case PduUtils.DCS_ENCODING_7BIT:
			...
			System.arraycopy(septets, septetUDHLength, septetsNoUDH, 0,
					septetsNoUDH.length);
		...
		throw new RuntimeException("Invalid dataCodingScheme: "
				+ getDataCodingScheme());
	}
	...
	public final String toString() {
		...
		if (smscAddress != null) {
			sb.append("SMSC Address: [Length: " + getSmscInfoLength() + " ("
					+ PduUtils.byteToPdu((byte) getSmscInfoLength())
					+ ") octets");
			sb.append(", Type: " + PduUtils.byteToPdu(smscAddressType) + " ("
					+ PduUtils.byteToBits((byte) smscAddressType) + ")");
		...
		if (udData != null) {
			switch (PduUtils.extractDcsEncoding(getDataCodingScheme())) {
			case PduUtils.DCS_ENCODING_7BIT:
				sb.append("User Data Length: " + getUDLength() + " ("
						+ PduUtils.byteToPdu(getUDLength()) + ") septets");
				sb.append("\n");
				break;
			case PduUtils.DCS_ENCODING_8BIT:
			case PduUtils.DCS_ENCODING_UCS2:
				sb.append("User Data Length: " + getUDLength() + " ("
						+ PduUtils.byteToPdu(getUDLength()) + ") octets");
				sb.append("\n");
				break;
			}
			...
			if (hasTpUdhi()) {
				// raw udh
				sb.append("User Data Header (pdu) : "
						+ PduUtils.bytesToPdu(getUDHData()));
				sb.append("\n");
				int udhLength = getUDHLength();
				sb.append("User Data Header Length: " + udhLength + " ("
						+ PduUtils.byteToPdu(udhLength) + ") octets");
				sb.append("\n");
				sb.append("\n");
				...
				// raw binary (as pdu)
				sb.append("\n");
				sb.append("Non UDH Data (pdu)    : "
						+ PduUtils.bytesToPdu(getUserDataAsBytes()));
				sb.append("\n");
				if (!isBinary()) {
