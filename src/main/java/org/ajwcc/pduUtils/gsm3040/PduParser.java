...
public class PduParser {
	...
	private Calendar readTimeStamp() {
		...
		int timezone = PduUtils.swapNibbles(timestamp) & 0x7F; // remove last
																// bit since
																// this is just
																// a sign
		...
		TimeZone tz = null;
		if (negative) {
			// bit 3 of unswapped value represents the sign (1 == negative, 0 ==
			// positive)
			...
			String gmtString = "GMT-" + hours + ":" + (minutes < 10 ? "0" : "")
					+ minutes;
	...
	private String readAddress(int addressLength, int addressType) {
		...
		if (addressLength > 0) {
			...
			int addressDataOctetLength = addressLength / 2
					+ ((addressLength % 2 == 1) ? 1 : 0);
			// extract data and increment position
			byte[] addressData = new byte[addressDataOctetLength];
			System.arraycopy(pduByteArray, position, addressData, 0,
					addressDataOctetLength);
			...
			case PduUtils.ADDRESS_TYPE_ALPHANUMERIC:
				...
				byte[] uncompressed = PduUtils
						.encodedSeptetsToUnencodedSeptets(addressData);
				int septets = addressLength * 4 / 7;
				byte[] choppedAddressData = new byte[septets];
				System.arraycopy(uncompressed, 0, choppedAddressData, 0,
						septets);
	...
	private Pdu parseStart() {
		...
		if (addressLength > 0) {
			...
			String smscAddress = readAddress((addressLength - 1) * 2,
					addressType);
	...
	private void parseUserData(Pdu pdu) {
		// ud length
		// NOTE: - the udLength value is just stored, it is not used to
		// determine the length
		// of the remaining data (it may be a septet length not an octet length)
		// - parser just assumes that the remaining PDU data is for the
		// User-Data field
		...
		if (pdu.hasTpUdhi()) {
			...
			while (position < endUdh) {
				...
				InformationElement ie = InformationElementFactory
						.createInformationElement(iei, ieData);
				pdu.addInformationElement(ie);
				position = position + iedl;
				if (position > endUdh) {
					// at the end, position after adding should be exactly at
					// endUdh
					throw new RuntimeException(
							"UDH is shorter than expected endUdh=" + endUdh
									+ ", position=" + position);
				}
			}
		}
