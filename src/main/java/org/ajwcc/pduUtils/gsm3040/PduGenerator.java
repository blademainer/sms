...
public class PduGenerator {
	...
	protected void writeSmscInfo(Pdu pdu) throws Exception {
		if (pdu.getSmscAddress() != null) {
			writeBCDAddress(pdu.getSmscAddress(), pdu.getSmscAddressType(),
					pdu.getSmscInfoLength());
		} else {
			writeByte(0);
		}
	}
	...
	protected void writeUDH(Pdu pdu, ByteArrayOutputStream udhBaos)
			throws IOException {
		...
		for (Iterator<InformationElement> ieIterator = pdu
	...
	protected void checkForConcat(Pdu pdu, int lengthOfText, int maxLength,
			int maxLengthWithUdh, int mpRefNo, int partNo) {
	...
	protected void writeUDData7bit(Pdu pdu, int mpRefNo, int partNo)
			throws Exception {
		...
		int potentialUdhLength = PduUtils
				.getNumSeptetsForOctets(computePotentialUdhLength(pdu));
		...
		byte[] textSeptets = getUnencodedSeptetsForPart(pdu, maxMessageLength,
				partNo);
	...
	private byte[] getUnencodedSeptetsForPart(Pdu pdu, int maxMessageLength,
			int partNo) {
		// computes offset to which part of the string is to be encoded into the
		// PDU
		// also sets the MpMaxNo field of the concatInfo if message is
		...
		// multi-septet
		byte[] unencodedSeptets = PduUtils.stringToUnencodedSeptets(pdu
				.getDecodedText());
	...
	protected void writeUDData8bit(Pdu pdu, int mpRefNo, int partNo)
			throws Exception {
	...
	protected void writeUDDataUCS2(Pdu pdu, int mpRefNo, int partNo)
			throws Exception {
		String decodedText = pdu.getDecodedText();
		// partNo states what part of the unencoded text will be used
		// - max length is based on the size of the UDH
		...
		checkForConcat(pdu, decodedText.length(),
		...
		// compute which portion of the message will be part of the message
		int offset = computeOffset(pdu, maxMessageLength, partNo);
		String textToEncode = decodedText.substring(offset,
				Math.min(offset + maxMessageLength, decodedText.length()));
		// generate udlength
		// based on partNo
		// udLength is an octet count for 8bit/ucs2
	...
	public String generatePduString(Pdu pdu, int mpRefNo, int partNo) {
		try {
			...
			case PduUtils.TP_MTI_SMS_DELIVER:
				generateSmsDeliverPduString((SmsDeliveryPdu) pdu, mpRefNo,
						partNo);
				break;
