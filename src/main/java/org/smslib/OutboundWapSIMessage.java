...
public class OutboundWapSIMessage extends OutboundBinaryMessage {
	...
	public OutboundWapSIMessage(String myRecipient, URL myUrl,
			Date myCreateDate, Date myExpireDate, WapSISignals mySignal,
			String myIndicationText) {
		super();
		this.url = myUrl;
	...
	public OutboundWapSIMessage(String myRecipient, URL myUrl, String text) {
		this(myRecipient, myUrl, new java.util.Date(), new java.util.Date(),
				WapSISignals.MEDIUM, text);
	}

	public OutboundWapSIMessage(String myRecipient, URL myUrl, String text,
			Date myExpireDate, WapSISignals mySignal) {
		this(myRecipient, myUrl, new java.util.Date(), myExpireDate, mySignal,
				text);
	}
	...
	protected WapSiPdu createPduObject() {
		WapSiPdu pdu;
		if (getStatusReport()) {
			pdu = PduFactory.newWapSiPdu(PduUtils.TP_SRR_REPORT
	...
	public void setDataBytes(byte[] b) {
		throw new RuntimeException(
				"setDataBytes() not supported for WapSi Message");
	}

	@Override
	public void addDataBytes(byte[] b) {
		throw new RuntimeException(
				"addDataBytes() not supported for WapSi Message");
	}
}
