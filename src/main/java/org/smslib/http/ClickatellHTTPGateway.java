...
public class ClickatellHTTPGateway extends HTTPGateway {
	...
	public boolean queryCoverage(OutboundMessage msg) throws TimeoutException,
		...
		url = new URL((this.secure ? this.HTTPS : this.HTTP)
				+ this.URL_COVERAGE);
		request.add(new HttpHeader("session_id", this.sessionId, false));
		if (msg.getRecipient().charAt(0) == '+')
			request.add(new HttpHeader("msisdn", msg.getRecipient()
					.substring(1), false));
		else
			request.add(new HttpHeader("msisdn", msg.getRecipient(), false));
		synchronized (this.SYNC_Commander) {
			response = HttpPost(url, request);
		}
	...
	public boolean sendMessage(OutboundMessage msg) throws TimeoutException,
		...
		boolean ok = false;
		if (this.sessionId == null) {
			Logger.getInstance().logError("No session defined.", null,
					getGatewayId());
		...
		try {
			if (msg.getType() == MessageTypes.OUTBOUND)
				...
				Logger.getInstance().logError("Incorrect message format.",
						null, getGatewayId());
				return false;
			}
			request.add(new HttpHeader("session_id", this.sessionId, false));
			if (msg.getRecipient().charAt(0) == '+')
				request.add(new HttpHeader("to", msg.getRecipient()
						.substring(1), false));
			else
