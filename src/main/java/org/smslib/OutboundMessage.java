...
public class OutboundMessage extends Message {
	...
	public Date getDispatchDate() {
		if (this.dispatchDate != null)
			return new java.util.Date(this.dispatchDate.getTime());
		return null;
	}
	...
	protected SmsSubmitPdu createPduObject() {
		...
		if (this.statusReport) {
			pdu = PduFactory.newSmsSubmitPdu(PduUtils.TP_SRR_REPORT
					| PduUtils.TP_VPF_INTEGER);
		} else {
	...
	protected void initPduObject(SmsSubmitPdu pdu, String smscNumber) {
		if ((getDstPort() > -1) && (getSrcPort() > -1)) {
			// port info
			pdu.addInformationElement(InformationElementFactory
					.generatePortInfo(getDstPort(), getSrcPort()));
		}
		// smscInfo
		...
		// set address type using the address (+ needs to be passed with it, if
		// present)
	...
	public String getPduUserData() {
		...
		// - if the user wishes to extract the UD per part, he would need to get
		// all pduStrings
		// using getPdus(String smscNumber, int mpRefNo), use a
		// PduParser on each pduString in the returned list, then access the UD
		// via the Pdu object
		List<String> pdus = pduGenerator.generatePduList(pdu, 1);
		// my this point, pdu will be updated with concat info (in udhi), if
		// present
		if ((pdu.hasTpUdhi()) && (getEncoding() == MessageEncodings.ENC7BIT)) {
	...
	public String getPduUserDataHeader() {
		...
		initPduObject(pdu, "");
		// NOTE: - the mpRefNo is arbitrarily set to 1
		// - if the user wishes to extract the UDH per part, he would need to
		// get all pduStrings
		// using getPdus(String smscNumber, int mpRefNo), use a
		// PduParser on each pduString in the returned list, then access the UDH
		// via the Pdu object
		List<String> pdus = pduGenerator.generatePduList(pdu, 1);
	...
	public void setEncoding(MessageEncodings encoding) {
		if (encoding == MessageEncodings.ENC8BIT) {
			if (this instanceof OutboundBinaryMessage)
				...
				throw new RuntimeException(
						"Cannot use 8-bit encoding with OutgoingMessage, use OutgoingBinaryMessage instead");
		} else {
			// 7-bit / ucs2
			super.setEncoding(encoding);
		}
	...
	public long getDeliveryDelay() {
		return (scheduledDeliveryDate == null) ? 0 : scheduledDeliveryDate
				.getTime() - System.currentTimeMillis();
	}

	public void setDeliveryDelay(long deliveryDelay) {
		scheduledDeliveryDate = new Date(System.currentTimeMillis()
				+ deliveryDelay);
