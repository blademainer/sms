...
public class ModemGateway extends AGateway {
	...
	public ModemGateway(ModemTypes myType, String id, String myModemDevice,
			int myModemParms, String myManufacturer, String myModel) {
		super(id);
		init(myType, myModemDevice, myModemParms, myManufacturer, myModel, null);
	}

	public ModemGateway(String id, String myModemDevice, int myModemParms,
			String myManufacturer, String myModel, AModemDriver myDriver) {
		super(id);
		init(null, myModemDevice, myModemParms, myManufacturer, myModel,
				myDriver);
	}

	private void init(ModemTypes myType, String myModemDevice,
			int myModemParms, String myManufacturer, String myModel,
		...
		setAttributes(AGateway.GatewayAttributes.SEND
				...
				| AGateway.GatewayAttributes.PORTADDRESSING
				| AGateway.GatewayAttributes.FLASHSMS
				| AGateway.GatewayAttributes.DELIVERYREPORTS);
		if (myDriver != null)
			setDriver(myDriver);
		else {
			if (myType == ModemTypes.SERIAL)
				setDriver(new SerialModemDriver(this, getModemDevice() + ":"
						+ getModemParms()));
			else
				setDriver(new IPModemDriver(this, getModemDevice() + ":"
						+ getModemParms()));
		}
	...
	public void startGateway() throws TimeoutException, GatewayException,
		...
		Logger.getInstance().logInfo(
				"Starting gateway, using " + getATHandler().getDescription()
						+ " AT Handler.", null, getGatewayId());
		getDriver().connect();
		Logger.getInstance()
				.logInfo(
						"Signal level/bit error rate: "
								+ getATHandler().getSignalLevel(), null,
						getGatewayId());
		Logger.getInstance().logInfo(
				"Network registration: "
						+ getATHandler().getNetworkRegistration(), null,
				getGatewayId());
		Logger.getInstance().logInfo(
				"Network operator: " + getATHandler().getNetworkOperator(),
				null, getGatewayId());
		super.startGateway();
	...
	public void stopGateway() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		Logger.getInstance().logInfo("Stopping gateway...", null,
				getGatewayId());
		getATHandler().done();
		super.stopGateway();
	...
	public InboundMessage readMessage(String memLoc, int memIndex)
		...
		synchronized (getDriver().getSYNCCommander()) {
			...
			for (InboundMessage msg : msgList)
				if ((msg.getMemIndex() == memIndex)
						&& (msg.getMemLocation().equals(memLoc)))
					return msg;
			return null;
		}
	}
	...
	public boolean deleteMessage(InboundMessage msg) throws TimeoutException,
		...
		synchronized (getDriver().getSYNCCommander()) {
			...
			else if ((msg.getMemIndex() == -1)
					&& (msg.getMpMemIndex().length() != 0)) {
	...
	@Override
	public int readPhonebook(Phonebook phonebook) throws TimeoutException,
			GatewayException, IOException, InterruptedException {
		int count = 0;
			...
			while (tokens.hasMoreTokens()) {
				...
				while (!entry.equalsIgnoreCase("OK")) {
					...
					phonebook.getContacts().add(
							new Contact(name, phone, location, Integer
									.parseInt(index)));
					count++;
					entry = reader.readLine().trim();
				}
	...
	private boolean deleteMessage(int memIndex, String memLocation)
			throws TimeoutException, GatewayException, IOException,
			InterruptedException {
		return getATHandler().deleteMessage(memIndex, memLocation);
	}

	private boolean sendMessageTEXT(OutboundMessage msg)
			throws TimeoutException, GatewayException, IOException,
			InterruptedException {
		int refNo;
		boolean ok = false;
		refNo = getATHandler().sendMessage(0, "", msg.getRecipient(),
				msg.getText());
		if (refNo >= 0) {
	...
	private void readMessagesTEXT(Collection<InboundMessage> msgList,
			MessageClasses msgClass, int myLimit) throws TimeoutException,
			GatewayException, IOException, InterruptedException {
		int i, j, memIndex;
		...
		for (int ml = 0; ml < (getATHandler().getStorageLocations().length() / 2); ml++) {
			if (getATHandler().switchStorageLocation(
					getATHandler().getStorageLocations().substring((ml * 2),
							(ml * 2) + 2))) {
				...
				while (true) {
					...
					memIndex = 0;
					try {
						memIndex = Integer.parseInt(line.substring(i + 1, j)
								.trim());
					} catch (NumberFormatException e) {
						// TODO: What to do here?
						Logger.getInstance().logWarn(
								"Incorrect Memory Index number parsed!", e,
								getGatewayId());
					}
					...
					if (Character.isDigit(tokens.nextToken().trim().charAt(0))) {
						...
						cal1.set(Calendar.YEAR, 2000 + Integer.parseInt(dateStr
								.substring(0, 2)));
						...
						cal1.set(Calendar.SECOND,
								Integer.parseInt(dateStr.substring(6, 8)));
						dateStr = tokens.nextToken().replaceAll("\"", "");
						cal2.set(Calendar.YEAR, 2000 + Integer.parseInt(dateStr
								.substring(0, 2)));
						...
						cal2.set(Calendar.SECOND,
								Integer.parseInt(dateStr.substring(6, 8)));
						msg = new StatusReportMessage(refNo, memIndex,
								getATHandler().getStorageLocations().substring(
										(ml * 2), (ml * 2) + 2),
								cal1.getTime(), cal2.getTime());
						msg.setGatewayId(getGatewayId());
						Logger.getInstance().logDebug(
								"IN-DTLS: MI:" + msg.getMemIndex(), null,
								getGatewayId());
						msgList.add(msg);
						...
						msg = new InboundMessage(cal1.getTime(), originator,
								msgText, memIndex, getATHandler()
										.getStorageLocations().substring(
												(ml * 2), (ml * 2) + 2));
						msg.setGatewayId(getGatewayId());
						Logger.getInstance().logDebug(
								"IN-DTLS: MI:" + msg.getMemIndex(), null,
					...
					while (true) {
						// line = reader.readLine();
						line = ((tmpLine == null || tmpLine.length() == 0) ? reader
								.readLine() : tmpLine);
						if (line == null)
							break;
						line = line.trim();
	...
	private boolean sendMessagePDU(OutboundMessage msg)
			throws TimeoutException, GatewayException, IOException,
			InterruptedException {
		boolean ok = false;
		...
		for (String pdu : pdus) {
			...
			Logger.getInstance().logDebug(newPdu.toString(), null,
					getGatewayId());
			int j = pdu.length() / 2;
			...
			Logger.getInstance().logDebug("Sending Pdu: " + pdu, null,
					getGatewayId());
	...
	private void readMessagesPDU(Collection<InboundMessage> msgList,
			MessageClasses messageClass, int myLimit) throws TimeoutException,
			GatewayException, IOException, InterruptedException {
		int i, j, memIndex;
		...
		for (int ml = 0; ml < (getATHandler().getStorageLocations().length() / 2); ml++) {
			if (getATHandler().switchStorageLocation(
				...
				// TEST
				// response =
				// "+CMGL: 0,1,,24\r07914477000000000414D0E10D6FE3DBA036A94D190000217050321314042030D82C2703B540F4F29C1E83815AA0A73C7D4EBBC3F4B71C442DCFE9\rOK\r";
				// TEST
				response = response.replaceAll("\\s+OK\\s+", "\nOK");
				reader = new BufferedReader(new StringReader(response));
				for (;;) {
				...
				while (true) {
					...
					if (line.length() <= 0 || line.equalsIgnoreCase("OK"))
						break;
					if (line.length() <= 0 || line.equalsIgnoreCase("ERROR"))
						break;
					i = line.indexOf(':');
					j = line.indexOf(',');
					memIndex = 0;
					...
					// last parameter is the length of the PDU, not counting the
					// addressing part
					// the parser must always have an addressing part -> we will
					// add it if required
					i = line.lastIndexOf(',');
					j = line.length();
					int pduSize = 0;
					try {
						pduSize = Integer.parseInt(line.substring(i + 1, j)
								.trim());
					} catch (NumberFormatException e) {
						// TODO: What to do here?
						Logger.getInstance()
								.logWarn("Incorrect pdu size parsed!", e,
										getGatewayId());
					}
					...
					try {
						Logger.getInstance().logDebug("READ PDU: " + pduString,
								null, getGatewayId());
						...
						if (pdu instanceof SmsDeliveryPdu) {
							Logger.getInstance().logDebug(pdu.toString(), null,
									getGatewayId());
							InboundMessage msg;
							String memLocation = getATHandler()
									.getStorageLocations().substring((ml * 2),
											(ml * 2) + 2);
							if (pdu.isBinary()) {
								msg = new InboundBinaryMessage(
										(SmsDeliveryPdu) pdu, memIndex,
										memLocation);
								if (Service.getInstance().getKeyManager()
										.getKey(msg.getOriginator()) != null)
									msg = new InboundEncryptedMessage(
											(SmsDeliveryPdu) pdu, memIndex,
											memLocation);
							} else {
								msg = new InboundMessage((SmsDeliveryPdu) pdu,
										memIndex, memLocation);
							}
							msg.setGatewayId(getGatewayId());
							Logger.getInstance().logDebug(
									"IN-DTLS: MI:" + msg.getMemIndex()
											+ " REF:" + msg.getMpRefNo()
											+ " MAX:" + msg.getMpMaxNo()
											+ " SEQ:" + msg.getMpSeqNo(), null,
									getGatewayId());
							if (msg.getMpRefNo() == 0) {
								// single message
								msgList.add(msg);
								...
								for (k = 0; k < this.mpMsgList.size(); k++) {
									...
									// check if current message list is for this
									// message
									if (listMsg.getMpRefNo() == msg
											.getMpRefNo()) {
										duplicate = false;
										// check if the message is already in
										// the message list
										for (l = 0; l < tmpList.size(); l++) {
											listMsg = tmpList.get(l);
											if (listMsg.getMpSeqNo() == msg
													.getMpSeqNo()) {
												duplicate = true;
												break;
											}
							...
							msg = new StatusReportMessage(
									(SmsStatusReportPdu) pdu, memIndex,
									getATHandler().getStorageLocations()
											.substring((ml * 2), (ml * 2) + 2));
							msg.setGatewayId(getGatewayId());
							msgList.add(msg);
							incInboundMessageCount();
						} else {
							// this theoretically will never happen, but it
							// occasionally does with phones
							// like some Sony Ericssons (e.g. Z610i, SENT
							// messages are included in this list)
							// instead of throwing a RuntimeException, just
							// ignore any messages that are not of type
							// SmsDeliveryPdu
							// SmsStatusReportPdu
							if (this.displayIllegalReceivedMessages) {
								Logger.getInstance().logError(
										"Wrong type of PDU detected: "
												+ pdu.getClass().getName(),
										null, getGatewayId());
								Logger.getInstance().logError(
										"ERROR PDU: " + pduString, null,
										getGatewayId());
							}
						}
					} catch (Exception e) {
						...
						UnknownMessage msg;
						msg = new UnknownMessage(pduString, memIndex,
								getATHandler().getStorageLocations().substring(
										(ml * 2), (ml * 2) + 2));
						msg.setGatewayId(getGatewayId());
						msgList.add(msg);
						incInboundMessageCount();
						Logger.getInstance().logError(
								"Unhandled SMS in inbox, skipping...", e,
								getGatewayId());
						Logger.getInstance()
								.logError("ERROR PDU: " + pduString, null,
										getGatewayId());
					}
					while (true) {
						line = reader.readLine();
	...
	public String getMessageByIndex(int msgIndex) throws TimeoutException,
			GatewayException, IOException, InterruptedException {
		synchronized (getDriver().getSYNCCommander()) {
	...
	private void checkMpMsgList(Collection<InboundMessage> msgList)
		...
		mpMsg = null;
		Logger.getInstance().logDebug(
				"CheckMpMsgList(): MAINLIST: " + this.mpMsgList.size(), null,
				getGatewayId());
		for (k = 0; k < this.mpMsgList.size(); k++) {
			tmpList = this.mpMsgList.get(k);
			Logger.getInstance().logDebug(
					"CheckMpMsgList(): SUBLIST[" + k + "]: " + tmpList.size(),
					null, getGatewayId());
			listMsg = tmpList.get(0);
			found = false;
							...
							if (listMsg.getMpSeqNo() == 1) {
								...
								if (mpMsg != null) {
									if (mpMsg instanceof InboundBinaryMessage) {
										...
										mpMsgBinary.addDataBytes(listMsgBinary
												.getDataBytes());
									} else {
										// NEW
										...
										if (mpMsg.getEndsWithMultiChar()) {
											...
											Logger.getInstance()
													.logDebug(
															"Adjusting dangling multi-char: "
																	+ textToAdd
																			.charAt(0)
																	+ " --> "
																	+ PduUtils
																			.getMultiCharFor(textToAdd
																					.charAt(0)),
															null,
															getGatewayId());
											textToAdd = PduUtils
										...
										mpMsg.setEndsWithMultiChar(listMsg
		...
		for (List<InboundMessage> remainingList : this.mpMsgList) {
			for (InboundMessage msg : remainingList) {
				if (getAgeInHours(msg.getDate()) > Service.getInstance()
						.getSettings().HOURS_TO_ORPHAN)
					if (Service.getInstance().getOrphanedMessageNotification() != null)
						if (Service
								.getInstance()
								.getOrphanedMessageNotification()
								.process(
										Service.getInstance().getGateway(
												msg.getGatewayId()), msg) == true)
							deleteMessage(msg);
			}
		}
	}
	 ...
	 */
	public String getManufacturer() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		String response;
		synchronized (getDriver().getSYNCCommander()) {
	 ...
	 *             The call was interrupted.
	 */
	public String getModel() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		String response;
		synchronized (getDriver().getSYNCCommander()) {
	 ...
	 *             The call was interrupted.
	 */
	public String getSerialNo() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		String response;
		synchronized (getDriver().getSYNCCommander()) {
	 ...
	 *             The call was interrupted.
	 */
	public String getImsi() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	 ...
	 *             The call was interrupted.
	 */
	public String getSwVersion() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		String response;
		synchronized (getDriver().getSYNCCommander()) {
	...
	boolean getGprsStatus() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		synchronized (getDriver().getSYNCCommander()) {
			return (getATHandler().getGprsStatus()
					.matches("\\+CGATT[\\p{ASCII}]*1\\sOK\\s"));
		}
	}
	 ...
	 *             The call was interrupted.
	 */
	public int getBatteryLevel() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		String response;
		synchronized (getDriver().getSYNCCommander()) {
			...
			if (response.indexOf("ERROR") >= 0)
				return 0;
			Matcher m = Pattern.compile("\\+CBC: (\\d+),\\s*(\\d+)").matcher(
					response);
			if (m.find())
	 ...
	 *             The call was interrupted.
	 */
	public int getSignalLevel() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		String response;
		StringTokenizer tokens;
		Integer rssi_code;
	 ...
	 *             The call was interrupted.
	 */
	public String sendCustomATCommand(String atCommand)
	 ...
	 */
	@Override
	public String sendUSSDCommand(String ussdCommand) throws GatewayException,
	...
	public boolean sendUSSDRequest(USSDRequest request)
			...
			InterruptedException {
		synchronized (getDriver().getSYNCCommander()) {
			return getATHandler().sendUSSDRequest(
					Integer.toString(request.getResultPresentation()
							.getNumeric()), request.getContent(),
					Integer.toString(request.getDcs().getNumeric()));
		}
	}
