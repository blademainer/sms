...
public class CNMIDetector {
	private final static Pattern splitterPattern = Pattern
			.compile("(\\(\\d([-,]\\d)*\\)|\\d)");
	...
	String getBestMatch(String[] availableOptions, String[] searchedOptions) {
			...
			for (int j = 0; j < availableOptions.length; j++) {
				...
				if (search.equals(option)) {
					Logger.getInstance().logDebug(
							"CNMI: Found best match: " + search, null, null);
					return search;
				}
			}
		}
		...
		Logger.getInstance().logInfo(
				"CNMI: No best match, returning: " + bestAvailableOption, null,
				null);
	...
	String getMode() {
		if (Service.getInstance().getSettings().DISABLE_CMTI)
			return "0";
		return this.mode;
	}
	...
	public String toString() {
		return "AT+CNMI=" + getMode() + "," + getMt() + "," + getBm() + ","
				+ getDs() + "," + getBfr();
	}
