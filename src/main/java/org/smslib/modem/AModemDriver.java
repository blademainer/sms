...
public abstract class AModemDriver {
	...
	protected abstract void connectPort() throws GatewayException, IOException,
			InterruptedException;

	protected abstract void disconnectPort() throws IOException,
			InterruptedException;

	protected abstract void clear() throws IOException;

	protected void connect() throws TimeoutException, GatewayException,
		...
		synchronized (getSYNCCommander()) {
			try {
				...
				setKeepAlive(new KeepAlive(
						"KeepAlive [" + getGateway().getGatewayId() + "]",
						Service.getInstance().getSettings().SERIAL_KEEPALIVE_INTERVAL * 1000));
				setCnmiEmulationProcessor(new CNMIEmulatorProcessor(
						"CNMIEmulatorProcessor [" + getGateway().getGatewayId()
								+ "]",
						Service.getInstance().getSettings().CNMI_EMULATOR_INTERVAL * 1000));
				...
				if ((getGateway().getCustomInitString() != null)
						&& (getGateway().getCustomInitString().length() > 0)) {
				...
				while (true) {
					...
					while (response.indexOf("BUSY") >= 0) {
						Logger.getInstance().logDebug(
								"SIM found busy, waiting...", null,
								getGateway().getGatewayId());
						Thread.sleep(Service.getInstance().getSettings().AT_WAIT_SIMPIN);
						response = getGateway().getATHandler().getSimStatus();
					}
					if (response.indexOf("SIM PIN2") >= 0) {
						Logger.getInstance().logDebug("SIM requesting PIN2.",
								null, getGateway().getGatewayId());
						if ((getGateway().getSimPin2() == null)
								|| (getGateway().getSimPin2().length() == 0))
							throw new GatewayException(
									"The GSM modem requires SIM PIN2 to operate.");
						if (!getGateway().getATHandler().enterPin(
								getGateway().getSimPin2()))
							throw new GatewayException(
									"SIM PIN2 provided is not accepted by the GSM modem.");
						Thread.sleep(Service.getInstance().getSettings().AT_WAIT_SIMPIN);
						continue;
					} else if (response.indexOf("SIM PIN") >= 0) {
						Logger.getInstance().logDebug("SIM requesting PIN.",
								null, getGateway().getGatewayId());
						if ((getGateway().getSimPin() == null)
								|| (getGateway().getSimPin().length() == 0))
							throw new GatewayException(
									"The GSM modem requires SIM PIN to operate.");
						if (!getGateway().getATHandler().enterPin(
								getGateway().getSimPin()))
							throw new GatewayException(
									"SIM PIN provided is not accepted by the GSM modem.");
						Thread.sleep(Service.getInstance().getSettings().AT_WAIT_SIMPIN);
						continue;
						...
						Logger.getInstance()
								.logWarn(
										"Erroneous CPIN response, proceeding with defaults.",
										null, getGateway().getGatewayId());
						break;
					}
					Logger.getInstance().logWarn(
							"Cannot understand SIMPIN response: " + response
									+ ", will wait for a while...", null,
							getGateway().getGatewayId());
					Thread.sleep(Service.getInstance().getSettings().AT_WAIT_SIMPIN);
				}
				getGateway().getATHandler().echoOff();
				...
				if (!waitForNetworkRegistration())
					Logger.getInstance()
							.logWarn(
									"Network Registration failed, proceeding with defaults.",
									null, getGateway().getGatewayId());
				...
				if (getGateway().getATHandler().getStorageLocations().length() == 0) {
					try {
						...
						Logger.getInstance().logInfo(
								"MEM: Storage Locations Found: "
										+ getGateway().getATHandler()
						...
						Logger.getInstance()
								.logWarn(
										"Storage locations could *not* be retrieved, will proceed with defaults.",
										e, getGateway().getGatewayId());
					}
				}
				if (!getGateway().getATHandler().setIndications()) {
					Logger.getInstance().logWarn(
							"Callback indications were *not* set succesfully!",
							null, getGateway().getGatewayId());
					getCnmiEmulationProcessor().enable();
				} else {
					if (getGateway().getATHandler().getIndications().getMode()
							.equals("0"))
						getCnmiEmulationProcessor().enable();
				}
				if (getGateway().getProtocol() == Protocols.PDU) {
					...
					if (!getGateway().getATHandler().setTextProtocol())
						throw new GatewayException(
								"The GSM modem does not support the TEXT protocol.");
				}
	...
	public void write(String s) throws IOException {
		Logger.getInstance().logDebug("SEND :" + formatLog(s), null,
				getGateway().getGatewayId());
		write(s.getBytes());
	}
	...
	public String getResponse() throws GatewayException, TimeoutException,
			IOException, InterruptedException {
		return getResponse(AsyncEvents.NOTHING);
	}

	/*
	 * Version of getResponse that looks for a particular type of unsolicited
	 * response (e.g. USSDResponse) and returns response without triggering an
	 * event. Useful if you want to get a USSD response synchronously rather
	 * using the event mechanism. eventResponse is the type of unsolicited
	 * response to look for, set it to AsyncEvents.NOTHING to handle all
	 * unsolicited responses through the event mechanism.
	 */
	public String getResponse(AsyncEvents eventResponse)
			throws GatewayException, TimeoutException, IOException,
			InterruptedException {
		String response;
		byte c;
		setLastError(-1);
		StringBuffer buffer = new StringBuffer(Service.getInstance()
				.getSettings().SERIAL_BUFFER_SIZE);
		try {
			while (true) {
				while ((getCharQueue().peek() == 0x0a)
						|| (getCharQueue().peek() == 0x0d)) {
					getCharQueue().get();
				}
				while (true) {
					c = getCharQueue().get();
				...
				if (buffer.charAt(buffer.length() - 1) != 0x0d)
			...
			Logger.getInstance().logDebug("BUFFER: " + buffer, null,
					getGateway().getGatewayId());
			if (getGateway().getATHandler().isUnsolicitedResponse(
					buffer.toString())) {
				AsyncEvents event = getGateway().getATHandler()
						.processUnsolicitedEvents(buffer.toString());
				if (event == eventResponse
						&& eventResponse != AsyncEvents.NOTHING) {
					return buffer.toString();
				}
				if ((event == AsyncEvents.INBOUNDMESSAGE)
						|| (event == AsyncEvents.INBOUNDSTATUSREPORTMESSAGE)
						|| (event == AsyncEvents.INBOUNDCALL)
						|| (event == AsyncEvents.USSDRESPONSE))
			...
			if (response.matches(rxErrorWithCode)) {
				...
				if (m.find()) {
					try {
						if (m.group(1).equals("CME")) {
							...
							throw new GatewayException(
									"Invalid error response: " + m.group(1));
					} catch (NumberFormatException e) {
						Logger.getInstance()
								.logDebug(
										"Error on number conversion while interpreting response: ",
										null, getGateway().getGatewayId());
						throw new GatewayException(
								"Cannot convert error code number.");
					}
				} else
					throw new GatewayException(
							"Cannot match error code. Should never happen!");
				...
				setLastError(10000);
			Logger.getInstance().logDebug(
					"RECV :" + formatLog(buffer.toString()), null,
					getGateway().getGatewayId());
		} catch (InterruptedException e) {
			Logger.getInstance().logWarn("GetResponse() Interrupted.", e,
					getGateway().getGatewayId());
			throw e;
		} catch (TimeoutException e) {
			Logger.getInstance().logDebug(
					"Buffer contents on timeout: " + buffer, null,
					getGateway().getGatewayId());
			throw e;
		}
		return buffer.toString();
	...
	public void clearBuffer() throws IOException, InterruptedException {
		synchronized (getSYNCCommander()) {
			Logger.getInstance().logDebug("clearBuffer() called.", null,
					getGateway().getGatewayId());
	...
	protected boolean waitForNetworkRegistration() throws GatewayException,
		...
		while (true) {
			...
			case 0:
				Logger.getInstance().logError(
						"GSM: Auto-registration disabled!", null,
						getGateway().getGatewayId());
				throw new GatewayException(
						"GSM Network Auto-Registration disabled!");
			case 1:
				Logger.getInstance().logInfo(
						"GSM: Registered to home network.", null,
						getGateway().getGatewayId());
				return true;
			case 2:
				Logger.getInstance().logWarn(
						"GSM: Not registered, searching for network...", null,
						getGateway().getGatewayId());
				if (++retries == 6)
					throw new GatewayException(
			...
			case 3:
				Logger.getInstance().logError(
						"GSM: Network registration denied!", null,
						getGateway().getGatewayId());
				throw new GatewayException("GSM Network Registration denied!");
			case 4:
				Logger.getInstance().logError(
						"GSM: Unknown registration error!", null,
						getGateway().getGatewayId());
				throw new GatewayException("GSM Network Registration error!");
			case 5:
				Logger.getInstance().logInfo(
						"GSM: Registered to foreign network (roaming).", null,
						getGateway().getGatewayId());
				return true;
			case -1:
				Logger.getInstance().logInfo("GSM: Invalid CREG response.",
						null, getGateway().getGatewayId());
	...
	private class CharQueue {
		...
		public synchronized void put(byte c) {
			...
			if (System.getProperty("smslib.dumpqueues") != null)
				Logger.getInstance().logDebug(
						"IN READER QUEUE : " + (int) c + " / " + (char) c,
						null, getGateway().getGatewayId());
			notifyAll();
		}

		public synchronized byte get() throws TimeoutException,
				InterruptedException {
			byte c;
			if (this.buffer == null)
				this.buffer = new byte[Service.getInstance().getSettings().SERIAL_BUFFER_SIZE];
			while (true) {
				try {
					...
					if (getGateway().getStatus() == GatewayStatuses.STARTED)
						Logger.getInstance()
								.logWarn(
										"Ignoring InterruptedException in Queue.get().",
										null, getGateway().getGatewayId());
					else {
						Logger.getInstance()
								.logWarn(
										"Re-throwing InterruptedException in Queue.get() - should be during shutdown...",
										null, getGateway().getGatewayId());
			...
			while (true) {
				try {
					...
					if (getGateway().getStatus() == GatewayStatuses.STARTED)
						...
						Logger.getInstance()
								.logWarn(
										"Re-throwing InterruptedException in Queue.peek() - should be during shutdown...",
										e, getGateway().getGatewayId());
						throw new InterruptedException();
					}
				}
	...
	private class ModemReader extends Thread {
		public ModemReader() {
			...
			setDaemon(true);
			start();
			Logger.getInstance().logDebug("ModemReader thread started.", null,
					getGateway().getGatewayId());
		}
		...
		public void run() {
			...
			while (isConnected()) {
				try {
					...
					for (int i = 0; i < getGateway().getATHandler()
							.getUnsolicitedResponses().length; i++) {
						if (data.indexOf(getGateway().getATHandler()
								.getUnsolicitedResponse(i)) >= 0) {
							Thread.sleep(100);
			...
			Logger.getInstance().logDebug("ModemReader thread ended.", null,
					getGateway().getGatewayId());
		}
	}
	...
	private class KeepAlive extends AServiceThread {
		...
		public void process() throws Exception {
			try {
				...
				if (getGateway().getStatus() == GatewayStatuses.STARTED) {
					synchronized (getSYNCCommander()) {
						if (!isConnected())
							return;
						try {
							if (!getGateway().getATHandler().isAlive())
				...
				Logger.getInstance().logError("ModemDriver: KeepAlive Error.",
						e, getGateway().getGatewayId());
				getGateway().setStatus(GatewayStatuses.RESTART);
			}
		}
	...
	private class AsyncNotifier extends Thread {
		...
		public AsyncNotifier() {
			...
			setDaemon(true);
			start();
			Logger.getInstance().logDebug("AsyncNotifier thread started.",
					null, getGateway().getGatewayId());
		}
		...
		protected void setEvent(AsyncEvents event, String response) {
			synchronized (this.SYNC) {
				...
				Logger.getInstance().logDebug("Storing AsyncEvent: " + ev,
						null, getGateway().getGatewayId());
		...
		public void run() {
			...
			while (isConnected()) {
				try {
					...
					Logger.getInstance().logDebug(
							"Processing AsyncEvent: " + event, null,
							getGateway().getGatewayId());
					if (event.event == AsyncEvents.INBOUNDMESSAGE) {
						Logger.getInstance().logDebug(
								"Inbound message detected!", null,
								getGateway().getGatewayId());
						...
						Logger.getInstance().logDebug(
								"Inbound status report message detected!",
								null, getGateway().getGatewayId());
						event.event = AsyncEvents.NOTHING;
						response = event.response;
						getAsyncMessageProcessor().setProcess();
					} else if (event.event == AsyncEvents.INBOUNDCALL) {
						Logger.getInstance().logDebug("Inbound call detected!",
								null, getGateway().getGatewayId());
						event.event = AsyncEvents.NOTHING;
						synchronized (getSYNCCommander()) {
						...
						Service.getInstance()
								...
								.getNotifyQueue()
								.add(new CallNotification(getGateway(),
										getOriginator(response)));
						// if (Service.getInstance().getCallNotification() !=
						// null)
						// Service.getInstance().getCallNotification().process(getGateway().getGatewayId(),
						// getOriginator(response));
					} else if (event.event == AsyncEvents.USSDRESPONSE) {
						Logger.getInstance().logDebug(
								"Inbound USSD response detected!", null,
								getGateway().getGatewayId());
						event.event = AsyncEvents.NOTHING;
						response = event.response;
						Logger.getInstance().logDebug(
								"USSD response : " + formatLog(response), null,
								getGateway().getGatewayId());
						if (Service.getInstance().getUSSDNotification() != null) {
							USSDResponse ussdResponse = new USSDResponse(
									response, getGateway().getGatewayId());
							ussdResponse.setContent(getGateway().getATHandler()
									.formatUSSDResponse(
											ussdResponse.getContent()));
							Service.getInstance().getUSSDNotification()
									.process(getGateway(), ussdResponse);
						}
					}
				} catch (InterruptedException e) {
					if (!isConnected())
					...
					Logger.getInstance().logInfo(
							"Invalid Message received! Ignoring. ", e,
							getGateway().getGatewayId());
			...
			Logger.getInstance().logDebug("AsyncNotifier thread ended.", null,
					getGateway().getGatewayId());
		}
	}
	...
	private class AsyncMessageProcessor extends Thread {
		...
		public AsyncMessageProcessor() {
			...
			setName("SMSLib-AsyncMessageProcessor : "
					+ getGateway().getGatewayId());
			setDaemon(true);
			start();
			Logger.getInstance().logDebug(
					"AsyncMessageProcessor thread started.", null,
					getGateway().getGatewayId());
		}
		...
		public void run() {
			while (isConnected()) {
				try {
					...
					synchronized (getSYNCInboundReader()) {
						getGateway().readMessages(this.msgList,
								MessageClasses.ALL);
			...
			Logger.getInstance().logDebug(
					"AsyncMessageProcessor thread ended.", null,
					getGateway().getGatewayId());
		}
	}
	...
	private class CNMIEmulatorProcessor extends AServiceThread {
		...
		public void process() throws Exception {
			if ((isConnected())
					&& (getGateway().getStatus() == GatewayStatuses.STARTED)) {
				synchronized (getSYNCInboundReader()) {
					this.msgList = new ArrayList<InboundMessage>();
	...
	public String getLastErrorText() {
		...
		else if ((getLastError() / 1000) == 5)
			return "CME Error " + (getLastError() % 1000);
		else if ((getLastError() / 1000) == 6)
	...
	protected void setAsyncMessageProcessor(
			AsyncMessageProcessor myAsyncMessageProcessor) {
	...
	protected void setCnmiEmulationProcessor(
			CNMIEmulatorProcessor myCnmiEmulationProcessor) {
