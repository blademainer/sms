...
public class ATHandler extends AATHandler {
	...
	@Override
	public void reset() throws TimeoutException, GatewayException, IOException,
	...
	@Override
	public void init() throws TimeoutException, GatewayException, IOException,
			InterruptedException {
	...
	@Override
	public void done() throws TimeoutException, GatewayException, IOException,
			InterruptedException {
		// No default behavior.
	}
	...
	@Override
	public String getSimStatus() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public boolean enterPin(String pin) throws TimeoutException,
	...
	@Override
	public boolean setVerboseErrors() throws TimeoutException,
			GatewayException, IOException, InterruptedException {
	...
	@Override
	public boolean setPduProtocol() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public boolean setTextProtocol() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public boolean setIndications() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		int RETRIES = 3;
		int count = 0;
		while (count < RETRIES) {
			...
			try {
				this.cnmiDetector = new CNMIDetector(getModemDriver()
						.getResponse());
				...
				Logger.getInstance().logWarn(
						"Retrying the detection of CNMI, modem busy?", null,
						getGateway().getGatewayId());
				Thread.sleep(Service.getInstance().getSettings().AT_WAIT_CNMI);
			}
		}
		Logger.getInstance().logWarn(
				"CNMI detection failed, proceeding with defaults.", null,
				getGateway().getGatewayId());
		return false;
	}
	...
	@Override
	public String getBatteryLevel() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public String getSignalLevel() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public String getNetworkOperator() throws TimeoutException,
			GatewayException, IOException, InterruptedException {
	...
	public void switchToCmdMode() throws IOException, InterruptedException {
		...
		while (new java.util.Date().getTime() - start.getTime() <= Service
				.getInstance().getSettings().AT_WAIT_CMD)
			Thread.sleep(100);
	}

	@Override
	public void keepLinkOpen() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	public int sendMessage(int size, String pdu, String phone, String text)
		...
		if (getGateway().getProtocol() == Protocols.PDU) {
			...
			while (true) {
				responseRetries = 0;
				getModemDriver().write(
						"AT+CMGS=\"_1_\"\r".replaceAll("\"_1_\"", "" + size));
				...
				while (!getModemDriver().dataAvailable()) {
					...
					if (responseRetries == Service.getInstance().getSettings().OUTBOUND_RETRIES)
						throw new GatewayException(
								"Gateway is not responding, max number of retries reached.");
					Logger.getInstance().logWarn(
							"ATHandler().SendMessage(): Still waiting for response (I) ("
									+ responseRetries + ")...", null,
							getGateway().getGatewayId());
				...
				if (getModemDriver().getLastError() == 0) {
					Matcher m = Pattern.compile("\\s*\\+CMGS: *(\\d+)")
							.matcher(response);
					if (m.find()) {
					...
					if (errorRetries == Service.getInstance().getSettings().OUTBOUND_RETRIES) {
						Logger.getInstance().logError(
								getModemDriver().getLastErrorText()
			...
			getModemDriver()
					.write("AT+CMGS=\"_1_\"\r".replaceAll("_1_", phone));
			getModemDriver().clearBuffer();
	...
	@Override
	public String listMessages(MessageClasses messageClass)
			throws TimeoutException, GatewayException, IOException,
			InterruptedException {
		if (getGateway().getProtocol() == Protocols.PDU) {
			if (messageClass == MessageClasses.ALL)
				getModemDriver().write("AT+CMGL=4\r");
			else if (messageClass == MessageClasses.UNREAD)
				getModemDriver().write("AT+CMGL=0\r");
			else if (messageClass == MessageClasses.READ)
	...
	@Override
	public boolean deleteMessage(int memIndex, String memLocation)
			throws TimeoutException, GatewayException, IOException,
			InterruptedException {
		if (!switchStorageLocation(memLocation))
			return false;
		Thread.sleep(Service.getInstance().getSettings().AT_WAIT);
		getModemDriver()
				.write("AT+CMGD=_1_\r".replaceAll("_1_", "" + memIndex));
		getModemDriver().getResponse();
		return (getModemDriver().isOk());
	}

	@Override
	public String getGprsStatus() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public String send(String s) throws TimeoutException, GatewayException,
			IOException, InterruptedException {
	...
	@Override
	public String getNetworkRegistration() throws GatewayException,
			...
			if (response.indexOf("+CPMS:") >= 0) {
				...
				while (tokens.hasMoreTokens()) {
					...
					if ((!loc.equalsIgnoreCase("MT"))
							&& ((getStorageLocations().indexOf(loc) < 0)))
	...
	@Override
	public String readPhonebookLocations() throws GatewayException,
			TimeoutException, IOException, InterruptedException {
		String response;
	...
	@Override
	public String readPhonebook(String loc) throws GatewayException,
		...
		try {
			...
			getModemDriver().write(
					"AT+CPBR=" + minIndex + "," + maxIndex + "\r");
			response = getModemDriver().getResponse();
			return response;
		} catch (Exception e) {
			Logger.getInstance().logWarn("Phonebook detection failed.", null,
					getGateway().getGatewayId());
			return "";
		}
	}
	...
	public String sendUSSDCommand(String ussdCommand, boolean interactive)
		...
		if (!interactive) {
			 ...
			 * further user input. We preemptively send an escape character
			 * which will terminate such a session.
			 */
			char esc = 0x1b;
			command = "" + esc + '\r';
	...
	@Override
	public boolean sendUSSDRequest(String presentation, String content,
			String dcs) throws GatewayException, TimeoutException, IOException,
			InterruptedException {
	...
	protected String formatUSSDCommand(String presentation, String content,
			String dcs) {
	...
	@Override
	public AsyncEvents processUnsolicitedEvents(String response)
			throws IOException {
		AsyncEvents event = AsyncEvents.NOTHING;
		if (response.indexOf(getUnsolicitedResponse(0)) >= 0)
			event = AsyncEvents.INBOUNDMESSAGE;
		else if (response.indexOf(getUnsolicitedResponse(1)) >= 0)
			event = AsyncEvents.INBOUNDSTATUSREPORTMESSAGE;
		else if (response.indexOf(getUnsolicitedResponse(2)) >= 0)
			event = AsyncEvents.NOTHING;
		else if (response.indexOf(getUnsolicitedResponse(3)) >= 0)
			event = AsyncEvents.INBOUNDCALL;
		else if (response.indexOf(getUnsolicitedResponse(4)) >= 0)
