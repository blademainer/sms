...
public class ATHandler_Telit extends ATHandler {
	public ATHandler_Telit(ModemGateway myGateway) {
		super(myGateway);
		Logger.getInstance().logInfo("ATHandler_Telit constructor.", null,
	...
	public boolean setIndications() throws TimeoutException, GatewayException,
		...
		try {
			...
			Logger.getInstance().logWarn("Error disabling messageindication",
					null, getGateway().getGatewayId());
		}
		Logger.getInstance().logInfo(
				"ATHandler_Telit: CNMI detectiondisabled.", null,
				getGateway().getGatewayId());
		return false;
	}
}
