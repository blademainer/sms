...
public class CommPortIdentifier {
	...
	static {
		try {
			classCommPortIdentifier = Class
					.forName("javax.comm.CommPortIdentifier");
		} catch (ClassNotFoundException e1) {
			try {
				classCommPortIdentifier = Class
						.forName("gnu.io.CommPortIdentifier");
	...
	public int getPortType() {
		try {
			Method method = classCommPortIdentifier.getMethod("getPortType",
	...
	public String getName() {
		try {
			Method method = classCommPortIdentifier.getMethod("getName",
					(java.lang.Class[]) null);
	...
	public SerialPort open(String appname, int timeout) {
		...
		try {
			Method method = classCommPortIdentifier.getMethod("open",
					paramTypes);
			return new SerialPort(method.invoke(this.realObject, appname,
					timeout));
		} catch (InvocationTargetException e) {
			throw new RuntimeException(new RuntimeException(e
					.getTargetException().toString()));
	...
	public static Enumeration<CommPortIdentifier> getPortIdentifiers() {
		...
		try {
			...
			Method method = classCommPortIdentifier.getMethod(
					"getPortIdentifiers", (java.lang.Class[]) null);
			CommPortIdentifier type = null;
			list = ReflectionHelper
					.invokeAndCastEnumeration(type, method, null);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
