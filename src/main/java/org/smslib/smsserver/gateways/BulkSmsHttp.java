...
public class BulkSmsHttp extends AGateway {
	public BulkSmsHttp(String myGatewayId, Properties myProps,
	...
	public void create() throws Exception {
		...
		if (getProperties().getProperty(propName + "region") == null) {
			setGateway(new org.smslib.http.BulkSmsHTTPGateway(getGatewayId(),
					getProperties().getProperty(propName + "username"),
					getProperties().getProperty(propName + "password")));
		} else {
			org.smslib.http.BulkSmsHTTPGateway.Regions Region = org.smslib.http.BulkSmsHTTPGateway.Regions.INTERNATIONAL;
			if (getProperties().getProperty(propName + "region")
					.equalsIgnoreCase("UNITEDKINGDOM"))
				Region = org.smslib.http.BulkSmsHTTPGateway.Regions.UNITEDKINGDOM;
			else if (getProperties().getProperty(propName + "region")
					.equalsIgnoreCase("SOUTHAFRICA"))
				Region = org.smslib.http.BulkSmsHTTPGateway.Regions.SOUTHAFRICA;
			else if (getProperties().getProperty(propName + "region")
					.equalsIgnoreCase("SPAIN"))
				...
				Region = org.smslib.http.BulkSmsHTTPGateway.Regions.USA;
					...
					.equalsIgnoreCase("GERMANY"))
				Region = org.smslib.http.BulkSmsHTTPGateway.Regions.GERMANY;
			setGateway(new org.smslib.http.BulkSmsHTTPGateway(getGatewayId(),
					getProperties().getProperty(propName + "username"),
					getProperties().getProperty(propName + "password"), Region));
		}
