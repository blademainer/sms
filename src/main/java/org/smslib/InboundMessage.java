...
public class InboundMessage extends Message {
	...
	public InboundMessage(Date date, String myOriginator, String text,
			int myMemIndex, String myMemLocation) {
	...
	public InboundMessage(MessageTypes type, int myMemIndex,
			String myMemLocation) {
	...
	public InboundMessage(SmsDeliveryPdu pdu, int myMemIndex,
			String myMemLocation) {
		...
		setMpSeqNo(0);
		setMpMemIndex(-1);
		int dcsEncoding = PduUtils
				.extractDcsEncoding(pdu.getDataCodingScheme());
		switch (dcsEncoding) {
		...
		if ((pdu.getAddressType() & PduUtils.ADDRESS_TYPE_ALPHANUMERIC) == PduUtils.ADDRESS_TYPE_ALPHANUMERIC)
			this.setOriginator(pdu.getAddress());
		else if ((pdu.getAddressType() & PduUtils.ADDRESS_TYPE_INTERNATIONAL) == PduUtils.ADDRESS_TYPE_INTERNATIONAL)
			this.setOriginator("+" + pdu.getAddress());
		else
			this.setOriginator(pdu.getAddress());
		...
		if (getEncoding() == MessageEncodings.ENC7BIT) {
			...
			byte[] temp = PduUtils.encodedSeptetsToUnencodedSeptets(pdu
					.getUDData());
			if (temp.length == 0) {
	...
	public void setMpMemIndex(int myMpMemIndex) {
		if (myMpMemIndex == -1)
			...
			this.mpMemIndex += (this.mpMemIndex.length() == 0 ? "" : ",")
					+ myMpMemIndex;
	}
	...
	public String toString() {
		...
		str += "\n";
		str += " Encoding: "
				+ (getEncoding() == MessageEncodings.ENC7BIT ? "7-bit"
						: (getEncoding() == MessageEncodings.ENC8BIT ? "8-bit"
								: "UCS2 (Unicode)"));
		str += "\n";
		str += " Date: " + getDate();
		str += "\n";
		...
		if (this instanceof InboundBinaryMessage) {
			...
			if (binaryMessage.getDataBytes() != null) {
				String binaryString = PduUtils.bytesToPdu((binaryMessage)
						.getDataBytes());
			...
			if (this instanceof InboundEncryptedMessage) {
				try {
					...
					str += " Message is **encrypted**, decrypted text: "
							+ encryptedMessage.getDecryptedText() + "\n";
				} catch (Exception e) {
		...
		if (this instanceof StatusReportMessage) {
			str += " Original Recipient: "
					+ ((StatusReportMessage) this).getRecipient();
			str += "\n";
			str += " Delivery Status: "
					+ ((StatusReportMessage) this).getStatus();
			str += "\n";
			...
			str += "\n";
			str += " Delivered Date: "
					+ ((StatusReportMessage) this).getReceived();
			str += "\n";
		}
		...
		str += "\n";
		str += " Source / Destination Ports: " + getSrcPort() + " / "
				+ getDstPort();
		str += "\n";
		str += "===============================================================================";
		str += "\n";
	...
	protected void extractData(SmsDeliveryPdu pdu) {
		...
		if (pdu.isBinary())
			throw new RuntimeException(
					"Trying to apply a binary pdu to an InboundMessage");
		this.setText(pdu.getDecodedText());
	}

	@Override
	public String getPduUserData() {
		if ((this.udh != null) && (getEncoding() == MessageEncodings.ENC7BIT)) {
			throw new RuntimeException(
					"getPduUserData() not supported for 7-bit messages with UDH");
		}
		return this.ud;
	}
