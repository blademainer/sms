...
public class KeyManager {
	...
	public void registerKey(String mobileNumber, AKey key) {
		this.keys.put(
				(mobileNumber.charAt(0) == '+' ? mobileNumber.substring(1)
						: mobileNumber), key);
	}
	...
	public AKey unregisterKey(String mobileNumber) {
		return this.keys.remove((mobileNumber.charAt(0) == '+' ? mobileNumber
				.substring(1) : mobileNumber));
	}
	...
	public AKey getKey(String mobileNumber) {
		if (mobileNumber == null)
			return null;
		return this.keys.get((mobileNumber.charAt(0) == '+' ? mobileNumber
				.substring(1) : mobileNumber));
	}
	 ...
	 */
	public byte[] encrypt(String mobileNumber, byte[] message)
			throws SMSLibException, InvalidKeyException,
			IllegalBlockSizeException, BadPaddingException,
			NoSuchPaddingException, NoSuchAlgorithmException {
		AKey k = getKey(mobileNumber);
		if (k == null)
			throw new SMSLibException(
					"Could not find Encryption Key for the specific number.");
		else if (k instanceof ASymmetricKey)
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public byte[] decrypt(String mobileNumber, byte[] message)
			throws SMSLibException, InvalidKeyException,
			IllegalBlockSizeException, BadPaddingException,
			NoSuchPaddingException, NoSuchAlgorithmException {
		AKey k = getKey(mobileNumber);
