	...
	public USSDResponse(String rawResp, String gtwId)
		...
		if (!matcher.matches())
			throw new InvalidMessageException(
					"Not a well-formed +CUSD response: |" + rawResp + "|");
		try {
			setGatewayId(gtwId);
			rawResponse = rawResp;
			sessionStatus = USSDSessionStatus.getByNumeric(Integer
					.valueOf(matcher.group(STATUS_INDEX)));
			if (matcher.groupCount() >= CONTENT_INDEX
					&& matcher.group(CONTENT_INDEX) != null) {
				setContent(matcher.group(CONTENT_INDEX));
			}
			if (matcher.groupCount() >= ENCODING_INDEX
					&& matcher.group(ENCODING_INDEX) != null) {
				setDcs(USSDDcs.getByNumeric(Integer.valueOf(matcher
						.group(ENCODING_INDEX))));
			}
		} catch (Exception e) {
			throw new InvalidMessageException("Session status: "
					+ matcher.group(STATUS_INDEX) + "; DCS: "
					+ matcher.group(ENCODING_INDEX));
		}
	}
