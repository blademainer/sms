...
public class InboundBinaryMessage extends InboundMessage {
	...
	public void addDataBytes(byte[] myDataBytes) {
		...
		System.arraycopy(myDataBytes, 0, newArray, this.dataBytes.length,
				myDataBytes.length);
		this.dataBytes = newArray;
	}

	public InboundBinaryMessage(SmsDeliveryPdu pdu, int memIndex,
			String memLocation) {
	...
	protected void extractData(SmsDeliveryPdu pdu) {
		...
		if (pdu.isBinary()) {
			...
			throw new RuntimeException(
					"Trying to apply a text pdu to an InboundBinaryMessage");
		}
	}
