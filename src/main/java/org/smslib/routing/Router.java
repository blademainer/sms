...
public class Router {
	...
	protected void preroute(OutboundMessage msg, Collection<AGateway> gateways) {
		for (AGateway gtw : gateways)
			if ((gtw.isOutbound())
					&& (gtw.getStatus() == GatewayStatuses.STARTED)) {
				if (msg.getGatewayId().equalsIgnoreCase("*"))
					getCandidates().add(gtw);
				else if (msg.getGatewayId()
						.equalsIgnoreCase(gtw.getGatewayId()))
					getCandidates().add(gtw);
			}
	}
	 ...
	 * @param msg
	 */
	public Collection<AGateway> route(OutboundMessage msg,
		...
		return getAllowed();
		/*
		 * // check if there are any gateways designated to send? if
		 * (getAllowed().size() > 0) gtw =
		 * getService().getLoadBalancer().balance(msg, getAllowed()); else {
		 * msg.setMessageStatus(MessageStatuses.FAILED);
		 * msg.setFailureCause(FailureCauses.NO_ROUTE); } // finish
		 * finishRouting(); return gtw;
		 */
	}
