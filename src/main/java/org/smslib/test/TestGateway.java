...
public class TestGateway extends AGateway {
	...
	InboundMessage generateIncomingMessage() {
		...
		InboundMessage msg = new InboundMessage(new java.util.Date(),
				"+1234567890", "Hello World! #" + getInboundMessageCount(), 0,
				null);
			...
			public void run() {
				while (!TestGateway.this.incomingMessagesThread.isInterrupted()) {
					synchronized (TestGateway.this.incomingMessagesThread) {
						try {
							TestGateway.this.incomingMessagesThread
									.wait(TestGateway.this.receiveCycle);
						} catch (InterruptedException e) {
							// NOOP
							break;
						}
					}
					if (!TestGateway.this.incomingMessagesThread
							.isInterrupted()) {
						...
						Service.getInstance()
								...
								.add(new InboundMessageNotification(
										getMyself(), MessageTypes.INBOUND,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.smslib.AGateway#readMessages(java.util.List,
	 * org.smslib.MessageClasses)
	 */
	@Override
	...
	public boolean sendMessage(OutboundMessage msg) throws TimeoutException,
		...
		Logger.getInstance().logInfo(
				"Sending to: " + msg.getRecipient() + " via: "
						+ msg.getGatewayId(), null, getGatewayId());
		this.counter++;
		if ((this.failCycle > 0) && (this.counter >= this.failCycle)) {
			this.counter = 0;
